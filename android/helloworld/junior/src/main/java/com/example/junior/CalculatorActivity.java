...
import com.example.junior.util.CalculateUtil;
...
public class CalculatorActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private boolean caculate() {
        if (operator.equals("＋")) { // 当前是相加运算
            result = String.valueOf(CalculateUtil.add(firstNum, nextNum));
        } else if (operator.equals("－")) { // 当前是相减运算
            result = String.valueOf(CalculateUtil.sub(firstNum, nextNum));
        } else if (operator.equals("×")) { // 当前是相乘运算
            result = String.valueOf(CalculateUtil.mul(firstNum, nextNum));
            ...
            if ("0".equals(nextNum)) { // 发现被除数是0
                ...
                result = String.valueOf(CalculateUtil.div(firstNum, nextNum));
            }
        }
        // 把运算结果打印到日志中
